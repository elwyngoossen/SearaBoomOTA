...
- **Hardware watchdog** — ESP32 task watchdog timer reboots the device automatically if `loop()` ever stalls for more than 30 seconds
- **Wi-Fi watchdog** — checks connection status every 15 seconds; reconnects automatically if dropped, and reboots if reconnection fails
- **Stream activity watchdog** — if audio stops while Wi-Fi is up, automatically reconnects to the stream after 30 seconds of silence
...
| 🔴 Red | Solid (brief) | Wi-Fi dropped — attempting to reconnect |
...
| `WDT_TIMEOUT_S` | `30` s | Hardware watchdog timeout before forced reboot |
